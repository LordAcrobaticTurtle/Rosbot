...
Rosbot::Rosbot() : 
    ...
    m_pendulumPID(
        &m_anglePidParams.currValue, &m_anglePidParams.response, &m_anglePidParams.target,
        m_anglePidParams.kp, m_anglePidParams.ki, m_anglePidParams.kd, DIRECT
    )
{ 
    ...
    m_pendulumPID.SetMode(AUTOMATIC);
    m_pendulumPID.SetOutputLimits(-255,255);
}
...
void Rosbot::resetRobot() {
    ...
    // for (int i = 0; i < m_state.numRows; i++) {
    //     for (int j = 0; j < m_state.numCols; j++) {
    //         m_state.data[i][j] = 0;
    //     }
    // }
...
void Rosbot::setMotorPosition (int motorIndex, float radps) {
    Serial.println("New target:" + String(radps));
    if (motorIndex == 0) {
        m_motorLPositionParams.target = radps;
        // m_motorL->setThrottle(throttle);
        // m_motorL->setPosition(m_motorLPositionParams);
        // m_motorLPositionParams.target = throttle;
    } else if (motorIndex == 1) {
        m_motorRPositionParams.target = radps;
...
RawSensors Rosbot::getLocalisationResponse() {
    RawSensors res;
    ...
    res.current.v1 = m_motorL->readCurrent();
    res.current.v2 = m_motorR->readCurrent();
...
void Rosbot::runLocalisation () {
    ...
    m_vwheel.v1 = m_encoderL->readRadsPerSecond();
    m_vwheel.v2 = m_encoderR->readRadsPerSecond();
}
...
void Rosbot::setAnglePIDParams (PIDParams params) {
    ...
    m_pendulumPID.SetTunings(params.kp, params.ki, params.kd);
}
...
void Rosbot::cascadedControl () {
    ...
    if (positionControlTimer.checkEnoughTimeHasPassed()) {
        ...
        m_motorLPositionParams.currValue = m_encoderL->readRadsPerSecond();
        m_motorRPositionParams.currValue = m_encoderR->readRadsPerSecond();

        PIDController::computeResponse(m_motorLPositionParams);
        PIDController::computeResponse(m_motorRPositionParams);
        Serial.println("Pos: " + String(m_motorLPositionParams.response));
    }
    
    // Feed position response in as target for angle controller. 

    if (angularControlTimer.checkEnoughTimeHasPassed()) {
        m_anglePidParams.target = (-m_motorLPositionParams.response) / 2.0;
...
void Rosbot::stateSpaceControl () {
    ...
    m_inputForce = m_pendulumControl.calculateControlSignal(m_desiredState, m_state);
    
    // Use m_inputForce to generate a torque command for the motors
    Matrix motorDesiredState(2,1);
    Matrix motorCurrentState(2,1);
    
    // Velocity desired and current state
    motorDesiredState.data[0][0] = m_motorRPositionParams.target * HardwareParameters::gearRatio;
    motorCurrentState.data[0][0] = m_encoderR->readRadsPerSecond() * HardwareParameters::gearRatio;
    
    // Torque desired and current state. (Actually flow of electrons current is the second state)
    // m_inputForce * HardwareParameters::wheelRadius / HardwareParameters::gearRatio / HardwareParameters::motorTorqueConstant

    // integral error is e = -Cx + r(t);

    motorDesiredState.data[1][0] = 0.0;
    motorCurrentState.data[1][0] = m_motorR->readCurrent();
    float voltageRight = m_motorControlRight.calculateControlSignal(motorDesiredState, motorCurrentState);
    m_motorR->setVoltage(-voltageRight);

    char stateBuffer[128];
    sprintf(stateBuffer, "[%f,%f]", motorCurrentState.data[0][0], motorCurrentState.data[1][0]);

    char desiredStateBuffer[128];
    sprintf(desiredStateBuffer, "[%f,%f]", motorDesiredState.data[0][0], motorDesiredState.data[1][0]);

    Serial.println("State: " + String(stateBuffer) + ", desired: " + String(desiredStateBuffer) + "V: " + String(voltageRight));
    // Serial.println("DesiredState: " + VoltageR: " + String(voltageRight) + ", inputForce: " + String(m_inputForce));
    // Check saturation bounds - This is done within the setVoltage function
    // m_motorL->setThrottle(m_inputForce);
    // m_motorR->setThrottle(-m_inputForce);
}

void Rosbot::pidControl () {
    m_anglePidParams.target = 0;
    m_anglePidParams.currValue = m_imuData.orientation.x;
    m_pendulumPID.Compute();
    m_motorL->setThrottle(-int(m_anglePidParams.response));
    m_motorR->setThrottle(int(m_anglePidParams.response));
    Serial.println("Response: " + String(m_anglePidParams.response));
}

void Rosbot::TankControl(double* channels) {
    // Get input from radio

    // Transform input from [0, 1] to [-1,1] on steering
    double mappedSteering = floatMap(channels[0], 0, 1, -1, 1);
    float leftCommand = channels[2]*PWM_MAX + 128*mappedSteering;
    float rightCommand = -(channels[2]*PWM_MAX - 128*mappedSteering);

    bool use_reverse = channels[7] > 0.7;

    leftCommand = use_reverse ? leftCommand*-1 : leftCommand;
    rightCommand = use_reverse ? rightCommand*-1 : rightCommand;

    m_motorL->setThrottle(leftCommand);
    m_motorR->setThrottle(rightCommand);
    // Serial.println()
...
void Rosbot::setStateSpaceGains (float posGain, float velGain, float aPosGain, float aVelGain) {
    m_K.data[0][0] = posGain;
    m_K.data[0][1] = velGain;
    m_K.data[0][2] = aPosGain;
    m_K.data[0][3] = aVelGain;
