...
float PIDController::computeResponse(PIDParams &params) {
    double error = params.currValue - params.target;
    double derror = (error - params.prevError)/params.dt;
    params.errorSum += error*params.dt;
    params.response = params.kp * error + params.kd * derror + params.ki * params.errorSum;
